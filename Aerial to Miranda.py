...
miranda = sst.Component("A0", "miranda.BaseCPU")
miranda.addParams({
    "clock_fequency"      : "2.6GHz",
    "max_reqs_cycle"      : 2,
...
memmgr = miranda.setSubComponent("memmgr", "ariel.MemoryManagerSimple")
memmgr.addParams({
...
for next_group in range(groups):
    ...
    for next_active_core in range(active_cores_per_group):
        ...
        miranda_cache_link = sst.Link("miranda_cache_link" + str(next_core_id))
        miranda_cache_link.connect( (miranda, "cache_link_" + str(next_core_id), ring_latency), (l1, "high_network_0", ring_latency) )
    ...
    for next_inactive_core in range(cores_per_group - active_cores_per_group):
        ...
        miranda_cache_link = sst.Link("miranda_cache_link_" + str(next_core_id))
        miranda_cache_link.connect( (miranda, "cache_link_" + str(next_core_id), ring_latency), (l1, "high_network_0", ring_latency) )
